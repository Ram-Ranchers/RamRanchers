...
public class BirdGen : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Vector3 newRamPos = spawnLanding;
        for (int i = 0; i < numBirds; i++)
        {
            newRamPos.x = spawnLanding.x + .05f * Random.Range(-numBirds, numBirds);
            newRamPos.z = spawnLanding.z + .05f * Random.Range(-numBirds, numBirds);
            birds[i] = Instantiate(birdPref, newRamPos, new Quaternion(0, 0, 0, 1), this.transform);
